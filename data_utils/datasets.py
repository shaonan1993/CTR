...
class NewBlockDataset(data.Dataset):
    def __init__(self, ds, tokenizer,
                 max_seq_len=1024,
                 sample_one_document=True,
                 non_sentence_start=0.0, filter_english=False, add_cls=True,
                 add_eos=True, sentence_end=True, **kwargs):
        ...
        self.ds = ds
        self.ds_len = len(self.ds)
        self.num_samples = 16 * self.ds_len
        self.max_seq_len = max_seq_len
        self.tokenizer = tokenizer
        self.sample_across_doc = not sample_one_document
        self.non_sentence_start = non_sentence_start
        ...
        # self.random_choose_sentence_from_doc = random_choose_sentence_from_doc
        """
        self.filter_english = filter_english
        if self.filter_english:
            import fasttext
    ...
    def get_random_sample(self, idx):
        item = self.ds[idx]
        tokens, loss_mask = item['tokens'], item['loss_masks']
        if self.add_eos:
            tokens = tokens + [self.tokenizer.get_command('eos').Id]
    ...
    def __getitem__(self, idx):
        ...
        # get possibly weighted random index from dataset
        ### todo: yanan
        cur_index = int(idx % self.ds_len)
        tokens, loss_mask = self.get_random_sample(cur_index)
        ...
        if self.sample_across_doc:
            while len(tokens) < self.max_seq_len:
                ### TODO: yanan
                cur_idx = rng.randint(self.ds_len)
                new_tokens, new_loss_mask = self.get_random_sample(cur_idx)
                # new_tokens, new_loss_mask = self.get_random_sample(rng)
                """
                if self.add_cls:
                    new_tokens = [self.tokenizer.get_command('ENC').Id] + new_tokens
    ...
    def right_strip_seq(self, tokens, loss_mask, seq_length):
        strip_right_tokens = len(tokens) - seq_length
        if strip_right_tokens > 0:
            while strip_right_tokens < len(tokens) - 1 and \
                    not self.contains_sentence_end(tokens[-strip_right_tokens - 1]):
                strip_right_tokens += 1
            if not self.sentence_end or len(tokens) - strip_right_tokens < seq_length // 2:
                strip_right_tokens = len(tokens) - seq_length
            tokens = tokens[:-strip_right_tokens]
