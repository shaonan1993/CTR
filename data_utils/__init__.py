...
from .datasets import split_ds, ConcatDataset, SplitDataset, LengthSamplingDataset, MultiSamplingDataset, XLDataset, \
    ScaleDataset, NewBlockDataset, BlockDataset
...
def make_dataset(path, seq_length, mem_length, shuffle=True, split=None, tokenizer=None,
    ...
    if ds_type.lower() != 'gpt-xl':
        ...
        _datasets = [[LengthSamplingDataset(ds) if ds is not None else None for ds in ds_split] for ds_split in 
        _datasets]
    """
    if dataset_temperature < 1.0:
        # _datasets = [MultiSamplingDataset(ds, reweight=True, temperature=dataset_temperature) if len(ds) > 1 else
        # ds[0] for ds in _datasets]
        _datasets = [
            MultiSamplingDataset(ds, reweight=True, temperature=dataset_temperature) if ds[0] is not None else None if len(
                ds) > 1 else ds[0] for ds in _datasets]
    else:
        # _datasets = [ConcatDataset(ds) if len(ds) > 1 else ds[0] for ds in _datasets]
        _datasets = [ConcatDataset(ds) if ds[0] is not None else None if len(ds) > 1 else ds[0] for ds in _datasets]
    ...
    def wrap_dataset(dataset):
        if ds_type.lower() == 'gpt-xl':
            ...
            ### TODO: yanan
            dataset = NewBlockDataset(dataset, tokenizer, max_seq_len=seq_length, **kwargs)
