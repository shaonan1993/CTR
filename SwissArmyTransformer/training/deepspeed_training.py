...
def setup_model_and_optimizer(args, model_cls, config_params=None, optimizer_cls=None):
    ...
    print_rank_0(f'debug: args.train_data: {args.train_data}, args.deepspeed: {args.deepspeed}')
...
def get_params_for_weight_decay_optimization(module):
    ...
    for module_ in module.modules():
        ...
        elif hasattr(module_, "get_params_for_weight_decay_optimization"):
            special_weight_decay_param = module_.get_params_for_weight_decay_optimization()
            if special_weight_decay_param:
                special_weight_decay_params.append(special_weight_decay_param)
        elif hasattr(module_, "no_optim_params"):
            pass
        else:
    ...
    if len(weight_decay_params['params']) == 0:
        if len(no_weight_decay_params['params']) == 0:
            assert len(special_weight_decay_params) != 0
            return special_weight_decay_params
        else:
            return (no_weight_decay_params, *special_weight_decay_params)
    elif len(no_weight_decay_params['params']) == 0:
        return (weight_decay_params, *special_weight_decay_params)

    return (weight_decay_params, no_weight_decay_params, *special_weight_decay_params)
...
def get_optimizer_param_groups(model):
    ...
    print_rank_0(
        f"number of trainable parameters on model parallel rank {mpu.get_model_parallel_rank()}: {num_parameters}")
    return param_groups
