...
from model.downstream import T5ForCloze
...
def multichoice_evaluate(model, dataloader, example_dict, args):
    ...
    if args.deepspeed:
        model = model.module
    if isinstance(model, T5Model):
        model = T5ForCloze(args, model=model)
    model.eval()
        ...
        for _, batch in enumerate(dataloader):
            ...
            if len(inputs[0].shape) == 3 and inputs[0].size(1) > segment_length:
                ...
                for i in range((num_choices - 1) // segment_length + 1):
                    input_batch = inputs[:num_enc_args] + [arg[:, i * segment_length: (i + 1) * segment_length] for arg in
                                                inputs[num_enc_args:]]
                    logits, *_ = model(*input_batch)
                    logit_list.append(logits)
                logits = torch.cat(logit_list, dim=1)
