...
def evaluate(data_iterator, model, args, timers, forward_step_func, verbose=False, **kwargs):
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            # Forward evaluation.
            ### TODO: Yanan
            lm_loss, mode = forward_step_func(data_iterator, model, args, timers, eval_step=iteration)
...
def train(
        ...
        train_data_iterator,
        val_data_iterator,
        timers, args, summary_writer=None, hooks={}):
    forward_step = hooks['forward_step']
    if 'save_codebook' in hooks:
        save_codebook = hooks['save_codebook']
    else:
        save_codebook = None
    model.train()
    ...
    while args.iteration < args.train_iters:
        ...
        if args.iteration % args.log_interval == 0:
            ...
            if args.deepspeed or args.DDP_impl == 'torch':
                timers.log(['forward', 'backward', 'optimizer', 'batch generator', 'data loader'],
                           normalizer=args.log_interval)
            else:
                timers.log(['forward', 'backward', 'allreduce', 'optimizer', 'batch generator', 'data loader'],
                           normalizer=args.log_interval)
            if save_codebook:
                save_codebook(args.iteration, args)
...
def get_train_val_test_data(args, tokenizer, name):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        train_data, val_data, test_data = data_config.apply(args, tokenizer, name=name)
