...
def add_training_args(parser):
    ...
    group.add_argument('--log-dir', type=str, default="", help="The directory to store the log")
...
def add_data_args(parser):
    ...
    group.add_argument('--reweight', action='store_true')

    group.add_argument('--eval-set', type=str, default='validation', choices=["validation", "test"])
    group.add_argument('--c4-as-external-data', action='store_true')

    ### TODO: yanan for TLM
    group.add_argument("--external_ratio", type=int)
    group.add_argument("--from-scratch", action='store_true', help="Whether to train from scratch.")
    group.add_argument("--external-multi-cache-dir", type=str)
    group.add_argument("--denoising-weight", type=float)
    group.add_argument("--lm-weight", type=float)
    group.add_argument("--task-weight", type=float)
    group.add_argument("--use_unique", action='store_true')
    group.add_argument("--use-adafactor", action='store_true')

    group.add_argument("--tlm-data-cache-dir", type=str, default="/sharefs/english/yanan/cache_dir")
    group.add_argument("--max-task-dataset-size", type=int, default=50000)

    # group.add_argument("--random-choose-sentence-from-doc", action='store_true')
    # group.add_argument("--training-task-type", type=str, default="all", choices=["gen", "cls", "all"])

    group.add_argument("--task-name", type=str)
    group.add_argument("--train-epoch", type=int)
    group.add_argument("--num-examples-per-prompt", type=int)
...
def add_finetune_config_args(parser):
    ...
    group.add_argument('--prompt-prefix-layers', type=int, default=None)
    group.add_argument('--prompt-weight-decay', type=float, default=None)
    group.add_argument('--prefix-prompt', type=int, default=0)
    group.add_argument('--use-kv-prompt', action='store_true')
...
def get_args(arg_list=None):
    ...
    if hasattr(args, "deepspeed") and args.deepspeed and args.deepspeed_config is not None:
        ...
        if args.log_interval is None:
            if "steps_per_print" in deepspeed_config:
                args.log_interval = deepspeed_config["steps_per_print"]
        else:
        ...
        if "optimizer" not in deepspeed_config:
